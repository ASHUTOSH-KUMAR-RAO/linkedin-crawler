...
function extractQueryHints(query: string) {
  ...
  if (!industry && lower.includes("fintech")) industry = "FinTech";
  if (!industry && lower.includes("saas")) industry = "SaaS";
  if (!industry && (lower.includes("ai") || lower.includes("artificial intelligence"))) industry = "AI/ML";
  return { location, industry };
}

export class DemoDiscoveryProvider implements DiscoveryProvider {
  readonly name = "demo-discovery";
